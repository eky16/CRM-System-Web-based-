...
if ($this->session->login['role'] == 'karyawan' OR $this->session->login['role'] == '') {
    ...
    <?php
		...
		public function index(){
			$this->data['title'] = 'Project Pending & Tender';
			...
			$this->load->view('leads/lihat_tender', $this->data);
		}
		public function ongoing(){
	    	...
	    	$this->data['reimbursment_count'] = count($this->m_reimburs->lihat_reimburs1()); // get resutl
	    	$this->data['reimbursment'] = $this->m_reimburs->lihat_reimburs1();
			$this->load->view('leads/lihat', $this->data);
		}
		...
		public function detail($id_lsp){
			...
			$this->data['grand_total_pesanan_preliminaries'] = $this->m_pembelian->laporan_total_preliminaries($id_lsp);
			...
			$this->load->view('leads/profil', $this->data); 
		}
		...
		public function save_laporan_status_log($id = NULL) { 
	     ...
	     //   exit;
	       ...
	       //print POST
	        ...
	        redirect('leads/detail/'.$link.'#section4'); //redirect page
	    }
	   ...
	   //    print_r ($_POST);
	    ...
	    //   echo '</pre>';
	     //   exit;
	       	...
	       	$this->session->set_flashdata('error', 'Status Log Project <strong>Gagal</strong> Ditambahkan!');
	        $this->session->set_flashdata('success', 'Status Log Project <strong>Berhasil</strong> Ditambahkan!');
	        redirect('leads/detail/'.$link.'#checklist'); //redirect page
	    }
	   ...
	   public function update_submit_boq($id = NULL) { 
	    ...
	    public function update_laporan_status_log($id = NULL) { 
	     ...
	     //   exit;
	        ...
	        redirect('leads/detail_checklist/'.$link); //redirect page
	    }
	    ...
	    //    exit;
			...
			$judul_issue = $this->input->post('judul_issue',TRUE);
	          ...
	          $this->m_kerja->create_package_issue($id_lsp_issue,$kode_issue,$ket_issu,$created_time_issue,$created_issue,$issue,$kode_issuee,$judul_issue);
	   ...
	   public function save_laporan_issue_hd($id = NULL) { 
		   	$link = $this->input->post('id_lsp_issue');

		   	$data['id_lsp_issue'] = $this->input->post('id_lsp_issue');
		   	$data['kode_issue'] = $this->input->post('kode_issue'); 
		   	$data['ket_issu'] = $this->input->post('ket_issu');
		   	$data['created_time_issue'] = $this->input->post('created_time_issue');
		   	$data['created_issue'] = $this->input->post('created_issue');
		   	$this->m_mom->tambah_laporan_issue_hd($data);
					//untuk log
	       ...
	       //print POST
	        ...
	        //exit;
	       	...
	       	$this->session->set_flashdata('error', 'Status Log Project <strong>Gagal</strong> Ditambahkan!');
	        $this->session->set_flashdata('success', 'Status Log Project <strong>Berhasil</strong> Ditambahkan!');
	        redirect('leads/detail/'.$link.'#issue'); //redirect page
	    }
		public function detail_foto($id_lsp,$tgl_upload){
			...
			$this->load->view('leads/detail_foto', $this->data);
		}
	  ...
	  //  $this->data['all_foto'] = $this->m_kerja->lihat_view_img($id_lsp,$tgl_upload);
			...
			$this->load->view('leads/detail_issue', $this->data);
		}
		public function detail_checklist($id_ceklist){
			...
			$this->load->view('leads/detail_checklist', $this->data);
		}
		public function tambah(){
			...
			$this->load->view('leads/tambah', $this->data);
		}
		public function save_problemsolve(){
	     ...
	     //   echo '</pre>';
	      //  exit;
	        ...
	        redirect('leads/detail_issue/'.$link); //redirect page
		...
		public function ubah($id_lsp){
			...
			$this->data['pm'] = $this->m_karyawan->get_atasan()->result();
			$this->data['dn'] = $this->m_karyawan->get_atasan()->result();
			$this->load->view('leads/ubah', $this->data);
		}
		...
		public function hapus($id_lsp){
			...
			public function hapus_issue_dt($id_sub_issue){
				...
				$data['user'] = $this->session->login['nama'];
				$data['waktu'] = date('Y-m-d H:i:s');
				$data['ket'] = 'Hapus Issue Detail';
				$data['kode'] = $id_sub_issue;
			//	$link = $kode_issue;

				$this->m_leads->tambah_log($data); //simpan ke tabel jenis izin

				if($this->m_leads->hapus_issue_dt($id_sub_issue)){
					$this->session->set_flashdata('success', 'Data leads <strong>Berhasil</strong> Dihapus!');
			//	redirect('leads/detail_issue/'.$link);
					redirect()->back(); 
				} else {
					$this->session->set_flashdata('error', 'Data leads <strong>Gagal</strong> Dihapus!');
			//	redirect('leads/detail_issue/'.$link);
		...
		public function hapus_stts_log($id_lsp){

			date_default_timezone_set('Asia/Jakarta');
			$data['user'] = $this->session->login['nama'];
			$data['waktu'] = date('Y-m-d H:i:s');
			$data['ket'] = 'Hapus Log Status Project';
			$data['kode'] = $id_lsp;
				$this->m_leads->tambah_log($data); //simpan ke tabel log

				if($this->m_leads->hapus2($id_lsp)){
					$this->session->set_flashdata('success', 'Data leads <strong>Berhasil</strong> Dihapus!');
					redirect('leads/detail/'.$id_lsp);
				} else {
					$this->session->set_flashdata('error', 'Data leads <strong>Gagal</strong> Dihapus!');
					redirect('leads');
				}
		...
		function delete_all_daily()
		{
			if($this->input->post('checkbox_value'))
			{
				...
				{
					$this->m_leads->deleted_daily($id[$count]);
				}
			}
		}
			...
			$this->data['all_stts_proyek'] = $this->m_kerja->lihat_status_log_proyek($id_lsp);
			$this->load->view('leads/material_lantai', $this->data);
		}
		...
		public function detail_material($id_ceklist){
	      ...
	      //   echo '</pre>';
	       // exit;
	        ...
	        redirect('leads/'.$link2.'/'.$link); //redirect page
	    }
	...
	//	 $this->data['data01'] = $this->m_leads->getData();
		...
		$this->load->view('leads/upload_bq', $this->data);
	}
public function importExcel(){
        ...
        try {
            ...
            for ($row = 2; $row <= $highestRow; $row++){                  //  Read a row of data into an array                 
                $rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row,
                 ...
                 $data = array(
                    "deskripsi_rap"=> $rowData[0][1],
                    "spesifikasi_rap"=> $rowData[0][2],
                    "jumlah_rap"=> $rowData[0][3],
                    "satuan_rap"=> $rowData[0][4],
                    "harga_rab"=> $rowData[0][5],
                    "total"=> $rowData[0][6],
                    "sisa_total"=> $rowData[0][7] * $rowData[0][3],
                    "harga_rap"=> $rowData[0][7],
                    "kode_rap"=> $kode,
                    "proyek_rap"=> $project,
                    "jenis_rap"=> $jenis_rap,
                    "total_rap"=> $rowData[0][3] * $rowData[0][7]
            ...
            if($insert){
				...
				redirect('leads/rab/'.$project); //redirect page
			}else{
				$this->session->set_flashdata('error', ' Terjadi Kesalahan !');
				 redirect('leads/upload_bq');
			}
   	...
   	public function rab($id_lsp = null){ 
	...
	//	$this->data['all_leads'] = $this->m_leads->lihat_id($id_lsp);
		...
		$this->load->view('leads/rab', $this->data);
		}
	...
	public function update_rap(){
     ...
     //   echo '</pre>';
      //  exit;
       ...
       $this->session->set_flashdata('success', 'Data <strong>Berhasil</strong> Diubah!');
        redirect('leads/rab/'.$id_proyek); //redirect page
        }else{
