...
class Wo extends CI_Controller {
	...
	$this->load->view('user/pembelian/tambah_jadwal_kirim', $this->data);
}
...
public function tambah_spk($id_lsp = NULL){
	...
	$kodeCSA = $nourut_csa + 1;
      ...
      public function keranjang_pengiriman(){
      	$this->load->view('user/pembelian/keranjang_pengiriman');
      }
...
for ($i = 0; $i < $jumlah_permintaan_barang; $i++) {
    ...
    $data_detail_keluar[$i]['merk'] = $this->input->post('merk_hidden')[$i];
    $data_detail_keluar[$i]['no_so'] = $this->input->post('no_so_hidden')[$i];
...
// yang status packing kiri itu nama field di tabel db nya, yang kanan itu yang dari program name yang dari keranjang
 ...
 // $this->db->insert('alba_permintaan_barang_dt',$data_detail_keluar);
			...
			redirect('user/wo/list_permintaan'); //redirect page
...
die();
      ...
      public function proses_tambah_pengiriman(){
      	...
      	$data_hd['tgl_dibuat']      = $createdtime_po;
			...
			redirect('user/wo/pengiriman'); //redirect page
		}
 ...
 public function kelengkapan_pr($id_pr = NULL){
	...
	public function export_excel_permintaan_diproses(){   
           $this->data = array( 'title' => 'KEMAJUAN PESANAN MARKETING ',
        		...
        		$this->load->view('user/pembelian/laporan/laporan_permintaan_proses', $this->data); 
    	}

    public function export_excel_list_so(){   
           $this->data = array( 'title' => 'LIST SALES ORDER ',
                'all_barang' => $this->m_pembelian->lihat_list_so());

        		$this->load->view('user/pembelian/laporan/laporan_list_so', $this->data); 
    	}
...
while ($jml_hari_packing > 0) {
    ...
    if (!isTanggalLiburAtauAkhirPekan($tgl_hasil_packing, $libur_nasional)) {
        $jml_hari_packing--;
			...
			redirect('user/wo/kelengkapan_pr/'.$id_hd);
			}
    ...
    public function update_siap_kirim(){
     ...
     //   exit;
			...
			$id_dt = $this->input->post('id_dt'); // Ambil ID permintaan barang
            $new_qty_delivery = (int) $this->input->post('Qty_Delivery'); // Ambil Qty Delivery baru

                    // Ambil data qty_order & qty_delivery sebelumnya
            $barang = $this->m_pembelian->get_barang_by_id($id_dt);
            $qty_order = isset($barang->quantity) ? (int) $barang->quantity : 0;
            $total_delivery = isset($barang->Qty_Delivery) ? (int) $barang->Qty_Delivery : 0;

                // Hitung total delivery setelah input baru
            $total_delivery += $new_qty_delivery;
            $qty_sisa = $qty_order - $total_delivery;

        // Validasi agar tidak melebihi Qty Order
        if ($qty_sisa >= 0) {
			$update_dt['detailNotes'] = $this->input->post('detailNotes');
			$update_dt['status_packing'] = $this->input->post('status_packing');
			$update_dt['qr_code'] = $this->input->post('qr_code');
            ...
            //$update_dt['no_wo'] = $this->input->post('no_wo');            
            $update_dt['Qty_Delivery'] = $total_delivery;
            $update_dt['Qty_Sisa'] = $qty_sisa;
			...
			$this->m_pembelian->update_qty_delivery($id_dt, $total_delivery, $qty_sisa);
				$this->m_pembelian->save_update_pr_dt($update_dt); //simpan ke tabel pr dt	
			$this->m_mom->tambah_log($data_log); //simpan ke tabel log 	
			$this->session->set_flashdata('success', 'Data Barang <strong>Berhasil</strong> Diubah!');
			redirect('user/wo/'.$id_hd);
			}
	...
	public function ubah_proses(){
     ...
     //   exit;
            ...
            $id_dt = $this->input->post('id_dt'); // Ambil ID permintaan barang
            $new_qty_partial_pack = (int) $this->input->post('qty_partial_packing'); // Ambil Qty Delivery baru

                    // Ambil data qty_order & qty_delivery sebelumnya
            $barang = $this->m_pembelian->get_barang_id($id_dt);
            $qty_order = isset($barang->quantity) ? (int) $barang->quantity : 0;
            $total_partial_pack = isset($barang->qty_partial_packing) ? (int) $barang->qty_partial_packing : 0;

                // Hitung total delivery setelah input baru
            $total_partial_pack += $new_qty_partial_pack;
            $qty_sisa_pack = $qty_order - $total_partial_pack;

            // Validasi agar tidak melebihi Qty Order
       if ($qty_sisa_pack < 0) {
    // Qty Partial melebihi Qty Order, tampilkan alert
    $this->session->set_flashdata('error', 'Qty Partial <strong>Melebihi</strong> Qty Order!');
    redirect('user/wo/'.$id_hd);  // Redirect kembali ke halaman sebelumnya
    } else {

	if ($cek_status === 'Packing') {
        ...
        $cek_status = $this->input->post('status_qr');
            ...
            $update_dt['tanggal_kirim'] = $this->input->post('tanggal_kirim');
            $update_dt['ketproblem'] = $this->input->post('ketproblem');
            $update_dt['qty_partial_packing'] = $total_partial_pack;
            $update_dt['qty_sisa_partial_pack'] = $qty_sisa_pack;
            ...
            $this->m_pembelian->update_qty_partial($id_dt, $total_partial_pack, $qty_sisa_pack);
				$this->m_pembelian->save_update_pr_dt($update_dt); //simpan ke tabel pr dt	
			$this->m_mom->tambah_log($data_log); //simpan ke tabel log 	
			$this->session->set_flashdata('success', 'Data Barang <strong>Berhasil</strong> Diubah!');
			...
			// Jika status QR adalah 'Ready'
    elseif ($cek_status === 'Ready') {
    ...
    if (!isTanggalLiburAtauAkhirPekan($tgl_hasil_packing, $libur_nasional)) {
     ...
     //   exit;
            ...
            if (!empty($_FILES['part_list']['name'])) {
                ...
                // Handle error jika upload gagal
                $error = array('error' => $this->upload->display_errors());
                print_r($error); // Tampilkan pesan error
                }
            }
			...
			$id_dt = $this->input->post('id_dt'); // Ambil ID permintaan barang
            $new_qty_partial_pack = (int) $this->input->post('qty_partial_packing'); // Ambil Qty partial baru
            ...
            $total_partial_pack = isset($barang->qty_partial_packing) ? (int) $barang->qty_partial_packing : 0;
                ...
                // Hitung total packing setelah input baru
            $total_partial_pack += $new_qty_partial_pack;
            $qty_sisa_pack = $qty_order - $total_partial_pack;
...
$no_terima = $this->input->post('no_wo');
    ...
    if ($status_qr === 'Packing') {
        ...
        $data_terima_dt['zona'] = $this->input->post('zona');
        $data_terima_dt['lantai'] = $this->input->post('lantai');
        $data_terima_dt['blok'] = $this->input->post('blok');

            $this->m_pembelian->save_proses_forecasttostok_dt($data_terima_dt);

        
            $this->m_pembelian->update_qty_partial($id_dt, $total_partial_pack, $qty_sisa_pack); //simpan alba permintaan barang dt
...
{
    $id_dt = $this->input->post('id_dt');
    $tipe  = $this->input->post('tipe'); // bisa 'gambar_kerja' atau 'part_list'

    if ($id_dt && in_array($tipe, ['gambar_kerja', 'part_list'])) {
        $this->db->where('id_dt', $id_dt)
                 ->update('alba_permintaan_barang_dt', [$tipe => null]);

        echo json_encode([
            'status' => 'success',
            'message' => ucfirst(str_replace('_',' ', $tipe)) . ' berhasil dihapus!'
        ]);
    } else {
        echo json_encode([
            'status' => 'error',
...
if (!empty($_FILES['gambar_kerja']['name'])) {
	...
	public function list_history_permintaan($id_lsp = NULL){
		...
		$this->load->view('user/pembelian/list_pr_01', $this->data);
	}
	public function move_pr(){
		...
		redirect('user/wo/proses_permintaan_barang');
	}
	...
	public function Subcon($id_lsp = NULL){
		...
		$this->load->view('user/pembelian/list_subcon', $this->data);
	}
    ...
    $this->load->view('user/pembelian/list_po_booking', $this->data);
}
	...
	public function pesanan_dibatalkan($id_lsp = NULL){ //Baru ditambahkan (8/28/23)
		...
		$this->load->view('user/pembelian/list_pesanan_dibatalkan', $this->data);
	}
	public function export_excel_batal(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_batal', $this->data); 
    	}
	public function export_excel_subcon(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_subcon', $this->data); 
    	}
	...
	public function export_excel_booking(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_stok_ready', $this->data); 
    	}
	public function export_excel_siap_kirim(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_siap_kirim', $this->data); 
    	}
	...
	public function pengiriman($id_lsp = NULL){
       ...
       $dan_tanggal = $this->input->post('dan_tanggal');
        ...
        // finish
	    $this->load->view('user/pembelian/list_siap_kirim', $this->data);
	       }
	...
	public function export_excel_produksi(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_produksi', $this->data); 
    	}
    ...
    public function export_excel_master_sp_actual_line(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_mps_actual', $this->data); 
    	}
    ...
    public function export_excel_master_sp_line(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_master_schedule_produksi', $this->data); 
    	}
    ...
    $this->load->view('user/pembelian/laporan/report_excel_msp_act', $this->data);
}
    ...
    //$jenis_line = isset($_GET['jenis_line']) ? $_GET['jenis_line'] : null; 
    $jenis_tanggal = isset($_GET['jenis_tanggal']) ? $_GET['jenis_tanggal'] : null;
    $jenis_produksi = ['Produksi', 'Forecast'];
    ...
    // Tentukan data berdasarkan filter
        if (empty($jenis_tanggal) || $jenis_tanggal == 'Semua Data' || ($jenis_tanggal == 'transDate' && !empty($tanggal))) {
            $this->data['all_pr'] = $this->m_pembelian->lihat_history_msp_01($tanggal, $dan_tanggal, $jenis_tanggal, $jenis_produksi);
    ...
    // Load view untuk menampilkan data yang akan diekspor
    $this->load->view('user/pembelian/laporan/laporan_msp', $this->data);
}
    ...
    // Tentukan data berdasarkan filter
        if (empty($jenis_tanggal) || $jenis_tanggal == 'Semua Data' || ($jenis_tanggal == 'transDate' && !empty($tanggal))) {
            $this->data['all_pr'] = $this->m_pembelian->lihat_history_msp($tanggal, $dan_tanggal, $jenis_tanggal,$jenis_produksi);
        } elseif (!empty($jenis_tanggal) && !empty($tanggal)) {
            $this->data['all_pr'] = $this->m_pembelian->lihat_history_msp($tanggal, $dan_tanggal, $jenis_tanggal,$jenis_produksi);
        } else {
            $this->data['all_pr'] = $this->m_pembelian->lihat_history_msp($tanggal, $dan_tanggal, $jenis_tanggal,$jenis_produksi);
    ...
    // URL untuk cetak PDF
    $url_cetak = 'user/wo/export1?filter=1&tanggal=' . $tanggal . '&dan_tanggal=' . $dan_tanggal;
    $this->data['url_cetak'] = base_url($url_cetak);

    // URL untuk ekspor Excel
    $url_cetak_excel = 'user/wo/export_excel_msp_act_01?filter=1&tanggal=' . $tanggal . '&dan_tanggal=' . $dan_tanggal . '&jenis_tanggal=' . $jenis_tanggal;
    $this->data['url_cetak_excel'] = base_url($url_cetak_excel);

    // Memuat view dengan data yang diperlukan
    $this->load->view('user/pembelian/laporan_msp_act', $this->data);
    // Debugging data untuk memastikan filter sudah bekerja
    //echo '<pre>';
    //print_r($this->data['all_pr']);
    //echo '</pre>';
    ...
    // URL untuk ekspor Excel
    $url_cetak_excel = 'user/wo/export_excel_msp_act?filter=1&tanggal=' . $tanggal . '&dan_tanggal=' . $dan_tanggal  . '&jenis_tanggal=' . $jenis_tanggal;
    $this->data['url_cetak_excel'] = base_url($url_cetak_excel);
    ...
    public function master_schedule_produksi_actual_line1($id_lsp = NULL){
		...
		$this->load->view('user/pembelian/master_schedule_produksi_actual', $this->data);
	}	
    ...
    // URL untuk ekspor Excel
    $url_cetak_excel = 'user/wo/export_excel_msp_01?filter=1&tanggal=' . $tanggal . '&dan_tanggal=' . $dan_tanggal . '&jenis_tanggal=' . $jenis_tanggal;
    $this->data['url_cetak_excel'] = base_url($url_cetak_excel);

    // Memuat view dengan data yang diperlukan
    $this->load->view('user/pembelian/laporan_msp', $this->data);
    // Debugging data untuk memastikan filter sudah bekerja
    //echo '<pre>';
    //print_r($this->data['all_pr']);
    //echo '</pre>';
    ...
    // URL untuk ekspor Excel
    $url_cetak_excel = 'user/wo/export_excel_msp?filter=1&tanggal=' . $tanggal . '&dan_tanggal=' . $dan_tanggal . '&jenis_tanggal=' . $jenis_tanggal;
    $this->data['url_cetak_excel'] = base_url($url_cetak_excel);
	...
	public function master_schedule_produksi_line1($id_lsp = NULL){
		...
		$this->load->view('user/pembelian/master_schedule_produksi_line', $this->data);
	...
	public function master_forecast($id_lsp = NULL){
		...
		//finish
		$this->load->view('user/pembelian/master_forecast', $this->data);
	}
    ...
    //$jenis_line = $this->input->post('jenis_line'); // <-- Ambil dari form input user
    $this->data['laporan'] = 'Produksi';
    //$this->data['jenis_line'] = $jenis_line; // <-- Simpan dalam data view
    ...
    $start_date = date('Y-m-01');  // Awal bulan
    $end_date = date('Y-m-t');  // Akhir bulan

     // Jika jenis_tanggal kosong, secara otomatis filter berdasarkan tanggal kirim bulan saat ini
    if (empty($jenis_tanggal)) {
        $jenis_tanggal = 'transDate';
        $tanggal = $start_date;
        $dan_tanggal = $end_date;
    }

    // Filter data berdasarkan input atau default ke tanggal kirim bulan saat ini
    if ($jenis_tanggal == 'Semua Data') {
        $this->data['all_pr'] = $this->m_pembelian->list_master_wo($jenis_produksi); 
    } elseif ($jenis_tanggal == 'tanggal_kirim') {
        $this->data['all_pr'] = $this->m_pembelian->list_master_wo_filter($tanggal, $dan_tanggal, $jenis_tanggal, $jenis_produksi); 
    } elseif ($jenis_tanggal == 'transDate') {
        $this->data['all_pr'] = $this->m_pembelian->list_master_wo_filter_transDate($tanggal, $dan_tanggal, $jenis_tanggal,$jenis_produksi); 
    }
	...
	public function export_excel_tambah_so(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_so', $this->data); 
    	}
	public function export_excel_master_forecast(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_master_forecast', $this->data); 
    	}
	public function export_excel_master_wo(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_master_wo', $this->data); 
    	}
	...
	public function produksi_line1($id_lsp = NULL){
		...
		$this->data['laporan'] = 'Produksi';
		$this->data['jenis_line'] = $jenis_line;
		$this->data['all_pr'] = $this->m_pembelian->list_produksi_line($jenis_line, $jenis_produksi); 
		$this->load->view('user/pembelian/list_po_hd', $this->data);
	}
	...
	public function selesai($id_lsp = NULL){
		...
		$this->load->view('user/pembelian/list_pengiriman_selesai', $this->data);
	}
   public function export_excel_laporan_item_selesai() {
    $this->data['title'] = "ALBA UNGGUL METAL";
    ...
    // Ambil data dari model
    $this->data['all_pr'] = $this->m_pembelian->lihat_history_pengiriman_selesai01($jenis_produksi, $tanggal, $dan_tanggal, $jenis_tanggal);
    ...
    // Load view untuk menampilkan data yang akan diekspor
    $this->load->view('user/pembelian/laporan/laporan_item_selesai', $this->data);
}
   ...
   public function export_excel_laporan_pengiriman() {
    ...
    // Load view untuk menampilkan data yang akan diekspor
    $this->load->view('user/pembelian/laporan/report_excel_pengiriman_selesai', $this->data);
}
     ...
     public function laporan_pengiriman($id_lsp = NULL)
{
    ...
    // URL untuk cetak PDF
    $url_cetak = 'user/wo/export1?filter=1&tanggal=' . $tanggal . '&dan_tanggal=' . $dan_tanggal  . '&type_barang_pesanan=' . $type_barang_pesanan;
    $this->data['url_cetak'] = base_url($url_cetak);

    // URL untuk ekspor Excel
    $url_cetak_excel = 'user/wo/export_excel_laporan_pengiriman?filter=1&tanggal=' . $tanggal . '&dan_tanggal=' . $dan_tanggal. '&type_barang_pesanan=' . $type_barang_pesanan;
    $this->data['url_cetak_excel'] = base_url($url_cetak_excel);

    // Memuat view dengan data yang diperlukan
    $this->load->view('user/pembelian/laporan_pengiriman_selesai', $this->data);
}
    ...
    public function item_selesai($id_lsp = NULL)
{
    ...
    set_time_limit(60); // Perpanjang waktu eksekusi jadi 60 detik

    // Default filter: tanggal kirim bulan berjalan jika tidak ada input
    if (empty($jenis_tanggal)) {
        $jenis_tanggal = 'tanggal_kirim';
        $tanggal       = date('Y-m-01'); // awal bulan
        $dan_tanggal   = date('Y-m-t');  // akhir bulan
    }
    ...
    if ($jenis_tanggal == 'Semua Data') {
        ...
        $this->data['all_pr'] = $this->m_pembelian->lihat_pengiriman_detail_filter(
            $jenis_produksi, $tanggal, $dan_tanggal, $jenis_tanggal
        ); 
    } elseif ($jenis_tanggal == 'transDate') {
        $this->data['all_pr'] = $this->m_pembelian->lihat_pengiriman_detail_filter_transDate(
            $jenis_produksi, $tanggal, $dan_tanggal, $jenis_tanggal); 
    } else {
        // fallback: gunakan query bawaan
        $this->data['all_pr'] = $this->m_pembelian->lihat_history_pengiriman_selesai01(
            $jenis_produksi, $tanggal, $dan_tanggal, $jenis_tanggal);
    }
    ...
    // URL untuk cetak PDF & Excel
    $query_string = http_build_query([
        'filter'        => 1,
        'tanggal'       => $tanggal,
        'dan_tanggal'   => $dan_tanggal,
        'jenis_tanggal' => $jenis_tanggal,
    ]);

    $this->data['url_cetak']       = base_url('user/wo/export1?' . $query_string);
    $this->data['url_cetak_excel'] = base_url('user/wo/export_excel_laporan_item_selesai?' . $query_string);

    // Load view
    $this->load->view('user/pembelian/list_pengiriman_selesai_item', $this->data);
}
	...
	public function export_excel_selesai(){ 
		...
		$this->load->view('user/pembelian/laporan/laporan_selesai', $this->data); 
    	}
	...
	public function detail_pr_01($id_pr = NULL){
		...
		$this->load->view('user/pembelian/detail_pr_01', $this->data); 
	}
	public function detail_pengiriman($id_pr = NULL){
		...
		$this->load->view('user/pembelian/detail_pengiriman', $this->data); 
	}
	...
	public function hapus_master_wo($id)
	    {
			...
			redirect('user/pembelian/master_wo'); //redirect page
		} else {
  ...
  //      print_r ($_POST);
   //     echo '</pre>';
    //   exit;
       ...
       redirect('user/wo/proses_permintaan_barang'); //redirect page
     }
	...
	public function ubah_statusdt($id)
{
    date_default_timezone_set('Asia/Jakarta');
    ...
    if (!empty($id)) {
        ...
        $this->db->update('alba_permintaan_barang_dt', ['status_dt' => 'OFF']);


       
        $this->session->set_flashdata('success', 'Permintaan Barang <strong>Berhasil</strong> Dinonaktifkan!');
        redirect('user/pembelian/list_permintaan');
    } else {
        $this->session->set_flashdata('error', 'Permintaan Barang <strong>Gagal</strong> Dihapus!');
        redirect('user/pembelian/list_permintaan');
    }
}
