			...
			<div id="content" data-url="<?= base_url('wo') ?>">
				...
				<div class="clearfix">
					...
					<div class="float-right">
						<a href="<?= base_url('wo/pengiriman') ?>" class="btn btn-secondary btn-sm"><i class="fa fa-reply"></i>&nbsp;&nbsp;Kembali</a>
					</div>
				</div>
				<hr>
				<div class="row">
					<div class="col">
						<div class="card shadow">
							...
							<div class="card-body">
								<form action="<?= base_url('wo/proses_tambah_pengiriman') ?>" id="form-tambah" method="POST">
									...
									<div class="form-row">
										<div class="form-group col-2">
											...
											$code = $codek ; ?>
											<input type="text" readonly name="kode_pengiriman" value="<?php $txt = sprintf("%04s", $kode_nik);
											$kodeTransaksi_pr = 'P/'.date('y').'/'.date('m').'/'.$txt;
										...
										<div class="form-group col-6">
											...
											<input type="date" id="tanggal_kirim" name="tgl_kiriman" value="<?= date('Y-m-d') ?>"  class="form-control">
										</div>
										<div class="form-group col-6">
...
inputElement.addEventListener("change", function() {
  var id2 = inputElement.value;
            $.ajax({
                    url : "<?php echo site_url('wo/get_data_barang_kirim_by_date');?>",
                    method : "POST",
                    data : {id2: id2},
                    async : true,
                    dataType : 'json',
		...
		$(document).ready(function(){
			...
			$('#id_dt').on('change', function(){
				...
				if($(this).val() == '') reset()
					...
					$.ajax({
						url: '<?php echo base_url()?>wo/get_data_barang_permintaan',
						type: 'POST',
						dataType: 'json',
						data: {id_dt: $(this).val()},
						success: function(data){
			...
			//	$('form :input').val('');
				...
				if(parseInt($('input[name="max_hidden"]').val()) <= parseInt(data_keranjang.quantity)) {
					...
					$.ajax({
						url: "<?php echo site_url('wo/keranjang_pengiriman');?>",
						type: 'POST',
						data: data_keranjang,
						success: function(data){
