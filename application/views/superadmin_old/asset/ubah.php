...
<input type="text" name="ket" placeholder="Masukkan Kode Barang" autocomplete="off"  class="form-control" required value="Ubah Asset" maxlength="8" hidden>
                           ...
                           <div class="form-group col-12">
                                     ...
                                     <select name="jenis_asset" class="form-control"  onchange="showTgl('hidden_tgl', this)" required>
                                        ...
                                        if (!empty($asst->jenis_asset)) {
                                            echo $asst->jenis_asset == 'Asset Bergerak' ? 'selected' : '';
                                        }
                                        
                                        ?>>Asset Bergerak</option>
                                        <option value="Asset Tidak Bergerak" <?php
                                        if (!empty($asst->jenis_asset)) {
                                            echo $asst->jenis_asset == 'Asset Tidak Bergerak' ? 'selected' : '';
                                        }
                                  ...
                                  <div class="form-group col-md-12">
                                            <label for="nama_barang"><strong>Nama Asset </strong></label>
                                             <input onkeyup="this.value = this.value.toUpperCase()" type="text" maxlength="50" name="nama_asset" placeholder="Masukkan Nama Asset" autocomplete="off" value="<?= $asst->nama_asset ?>"  class="form-control">
                                        </div>

                                  <?php if ($asst->jenis_asset == "Asset Bergerak"): ?>      
                                     <div class="form-group col-md-12">
                                            <label for="nama_barang"><strong>Tgl Pajak </strong></label>
                                             <input type="date" maxlength="50"  name="tgl_pajak" placeholder="Masukkan Nama Lengkap" autocomplete="off" value="<?= $asst->tgl_pajak ?>"  class="form-control">
                                </div><?php endif; ?>
  ...
  <div class="file-upload-content">
    ...
    <div class="image-title-wrap">
      <button type="button" onclick="removeUpload()" class="remove-image">Remove <span class="image-title">Uploaded Image</span></button>
                            ...
                            ?>>PERBAIKAN</option>
                              <option value="TERPAKAI" <?php
                            if (!empty($asst->status)) {
                                echo $asst->status == 'TERPAKAI' ? 'selected' : '';
                            }
                            ?>>TERPAKAI</option>
                            </select>
                                </div>
