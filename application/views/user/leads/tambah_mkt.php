			...
			<div id="content" data-url="<?= base_url('leads') ?>">
				...
				<div class="clearfix">
					...
					<div class="float-right">
						<a href="<?= base_url('user/leads') ?>" class="btn btn-secondary btn-sm"><i class="fa fa-reply"></i>&nbsp;&nbsp;Kembali</a>
					</div>
				</div>
				<hr>
				<div class="row">
					<div class="col-md-12">
						<div class="card shadow">
							...
							<div class="card-body">
			<form action="<?= base_url('user/leads/proses_tambah_mkt') ?>" id="from1" name="form" method="POST">
						...
						<select id="select-state" placeholder="project_manager" name="project_manager" class="form-control" >           
                            ...
                            <?php foreach ($pm as $spv) : ?>
                                <option value="<?php echo $spv->nama_karyawan ?>" <?php
                                if (!empty($emp->project_manager)) {
                                    echo $spv->EmployeeID == $emp->project_manager ? 'selected' : '';
                                }
                            ...
                            <?php foreach ($sm as $spv) : ?>
                                <option value="<?php echo $spv->nama_karyawan ?>" <?php
                                if (!empty($emp->site_manager)) {
                                    echo $spv->EmployeeID == $emp->site_manager ? 'selected' : '';
                                }
                                ?>><?php echo $spv->EmployeeID .' - '. $spv->nama_karyawan ?></option>
                                    <?php endforeach; ?>
						...
						<select id="select-state" placeholder="spv_manager" name="site_manager" class="form-control" >           
                            ...
                            <?php foreach ($spv1 as $spv) : ?>
                                <option value="<?php echo $spv->nama_karyawan ?>" <?php
                                if (!empty($emp->spv_manager)) {
                                    echo $spv->EmployeeID == $emp->spv_manager ? 'selected' : '';
                                }
                                ?>><?php echo $spv->EmployeeID .' - '. $spv->nama_karyawan ?></option>
                                    <?php endforeach; ?>
									...
									<div class="form-row">
										...
										<div class="form-group col-md-6">
											...
											<input type="text" name="name" placeholder="Masukkan Nama Project" autocomplete="off" maxlength="50"  class="form-control" >
