...
class M_penerimaan_mf extends CI_Model {
...
public function lihat_penerimaan() {
    ...
    $kondisi = "( ( ( alba_customer.nama_cst = 'Forecast' AND alba_permintaan_barang_dt.status_qr = 'selesai'
                      AND alba_permintaan_barang_dt.quantity > 0 ) ))";
    ...
    return $result;
        ...
        $this->db->where('alba_barang_penerimaan.tgl_terima BETWEEN 
            ...
            ');		
        $this->db->select('alba_barang_penerimaan.*', FALSE);
        $this->db->select('alba_barang_detail_terima_mf.*', FALSE);
        $this->db->from('alba_barang_detail_terima_mf');
        $this->db->join('alba_barang_penerimaan', 'alba_barang_penerimaan.no_terima = alba_barang_detail_terima_mf.no_terima_mf', 'left');
        $this->db->order_by('alba_barang_penerimaan.tgl_terima', 'DESC');
        $query_result = $this->db->get();
        $result = $query_result->result();
        return $result;
        ...
        $this->db->join('alba_barang_penerimaan', 'alba_barang_penerimaan.no_terima = detail_terima_mf.no_terima_mf', 'left');
        $this->db->order_by('alba_barang_penerimaan.tgl_terima', 'DESC');
        $query_result = $this->db->get();
        $result = $query_result->result();
        return $result;
	...
	public function lihat_no_terima1($no_terima_mf){
		return $this->db->get_where($this->_table, ['no_terima_mf' => $no_terima_mf])->row();
	}
